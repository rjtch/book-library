                ...
                try {
                    ...
                    const propsData = props.properties
                        ? Object.keys(props.properties).map(propName => {
                            ...
                            return Object.assign(Object.assign({}, propSchema), { name: propName, type: this.getType(propSchema), desc: [propSchema.title, propSchema.description].filter(s => s).join(' '), required: requiredPropKeys.some(key => key === propName) });
                        })
                        : [];
